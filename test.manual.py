...
def test_validate_data_with_nulls():
    ...
    try:
        ...
        print(f"   Advertencias deberían estar en logs")
        return True
...
def test_process_data_basic():
    ...
    # Crear CSV temporal para testear
    import tempfile
    import os
...
def test_fetch_data_timeout():
    ...
    print("   2. Ejecuta: python api_data_fetcher.py")
    ...
    return None  # No auto-ejecutable
...
def run_all_tests():
    ...
    for name, result in results:
        status = "✅ PASÓ" if result is True else "❌ FALLÓ" if result is False else "⚠️ MANUAL"
        print(f"{status} - {name}")
